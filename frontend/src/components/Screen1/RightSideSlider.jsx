...
//     // console.log(doctorsData, "doctors data in rigjt slider");

//     const normalizedList = useMemo(() => {
//         if (!doctorsData || doctorsData.length === 0) return [];

//         if (doctorsData.length === 1) {
//             return Array(12).fill(doctorsData[0]); // repeated for infinite scroll
//         }

//         if (doctorsData.length >= 1 && doctorsData.length <= 8) {
//             return [...doctorsData, ...doctorsData, ...doctorsData, ...doctorsData]; // repeated for infinite scroll
//         }
...
//     const speedPerCard = 1; // 4 seconds per card
//     const animationDuration = normalizedList?.length * speedPerCard;
...
//                 <div style={{ animationDuration: `${animationDuration}s` }} className="animate-vertical-scroll flex flex-col [@media(min-width:3200px)_and_(min-height:2000px)]:px-5 [@media(min-width:4200px)_and_(min-height:2600px)]:px-8 gap-4  [@media(min-width:3200px)_and_(max-height:2000px)]:gap-6  [@media(min-width:3200px)_and_(min-height:2000px)]:gap-8 [@media(min-width:4200px)_and_(min-height:2600px)]:gap-10">
//                     {[...normalizedList, ...normalizedList].map((row, i) => (
//                         <div
//                             key={i}
//                             className="bg-white/90 border border-blue-200/50 rounded-xl px-4 [@media(min-width:4200px)_and_(min-height:2600px)]:px-10 py-3  [@media(min-width:3200px)_and_(max-height:2000px)]:p-4  [@media(min-width:3200px)_and_(min-height:2000px)]:p-5 [@media(min-width:4200px)_and_(min-height:2600px)]:p-7 shadow-md  hover:bg-blue-50/50 hover:shadow-lg transition-all flex justify-between text-[15px] [@media(min-width:3200px)]:text-[20px]   [@media(min-width:3200px)_and_(min-height:2000px)]:text-[24px] [@media(min-width:4200px)_and_(min-height:2600px)]:text-[28px] font-semibold  text-blue-800
//           "
//                         >
//                             <span className="truncate w-[48%] text-left">{row?.DOCTOR_NAME}</span>
//                             <span className="truncate w-[48%] text-right text-blue-600">{row?.FACULTY_NAME}</span>
//                         </div>
//                     ))}
//                 </div>
//             </div>
