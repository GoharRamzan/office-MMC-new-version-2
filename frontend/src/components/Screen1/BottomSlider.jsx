...
const BottomSlider = () => {
    ...
    // Speed in pixels per second - bade screen pe mast lagta hai 70-80
    ...
    const cardWidthWithGap = 340; // 320px card + ~20px gap
    const totalWidth = UpdateDoctorsData.length * cardWidthWithGap;
    const duration = totalWidth / SCROLL_SPEED;

    if (!UpdateDoctorsData.length) return null;

    return (
        <div className="relative w-full h-[45vh] [@media(min-width:2200px)_and_(min-height:1200px)]:h-[25vh] [@media(min-width:3200px)_and_(min-height:2000px)]:h-[21vh] overflow-hidden bg-[#e0f2ff] border-t border-blue-200 py-6 [@media(min-width:2200px)_and_(min-height:1200px)]:py-12 mb-[45px]">

            {/* Scrolling Container */}
            <div
                className="flex gap-4 h-full [@media(min-width:2200px)_and_(min-height:1200px)]:gap-6 animate-horizontal-scroll-d"
                style={{
            ...
            >
                ...
                {[...UpdateDoctorsData, ...UpdateDoctorsData].map((doc, i) => (
                    <div
                        key={`${doc.DOCTOR_ID}-${i}`}
                        className="bg-white/90 backdrop-blur-sm border h-full border-blue-100 rounded-2xl p-4 
                       [@media(min-width:3200px)_and_(min-height:2000px)]:p-8 
                       shadow-md flex items-center gap-4 
                       [@media(min-width:3000px)_and_(min-height:2000px)]:gap-10 
                    ...
                    >
                        ...
                        {/* Doctor Info */}
                        <div className="flex flex-col [@media(min-width:2200px)_and_(min-height:1200px)]:gap-3 
